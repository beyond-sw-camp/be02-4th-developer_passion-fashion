...
#### 기존 프로젝트 현황
- 조원들이 Git에 push 할 때 코드에 대한 신뢰성을 보장 할 수 없어 매번 테스트를 새롭게 해야했고 이에 따른 시간 소요가 많았다. (CI 필요성 대두)
- 직접 빌드를 하고 배포용 컴퓨터에 배포파일을 옮겨 배포를 할 때 단순반복적인 일이지만 의외로 많은 시간이 소요 됐다. (CD 필요성 대두)
